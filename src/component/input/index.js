...
class Input extends Component {
  ...
  renderAlert = () => {
    ...
    if (state[`${name.toLowerCase()}`] === "" && state["submitStatus"])
      return <span className="textALert">*Wajib Diisi</span>;
  ...
  render() {
    ...
    return (
      <>
        <div
          className={`input-div ${state[`isFocus${name}`] ? "focus" : ""} ${
            state[`${name.toLowerCase()}`] === "" && state["submitStatus"]
              ? "redAlert"
              : ""
          }`}
        >
