...
class DetailMahasiswaPage extends Component {
  ...
  getNilaiIPK = () => {
    ...
    if (ipkSekarang !== 0.0 && this.getNilaiIPS() !== 0.0) {
    ...
    return (
      <>
        ...
        <Link to="/list-mahasiswa">
          <button style={{ marginLeft: 100, cursor: "pointer" }}>back</button>
        </Link>
        ...
        <h3 style={{ marginLeft: 100 }}>
          IPS : {this.getNilaiIPS().toFixed(2)}
        </h3>
        <h3 style={{ marginLeft: 100 }}>
          IPK : {this.getNilaiIPK().toFixed(2)}
        </h3>
        ...
        <table className="customers-list" width="80%">
          ...
          <tbody>{this.renderPageDetail()}</tbody>
        </table>
      </>
    );
  }
